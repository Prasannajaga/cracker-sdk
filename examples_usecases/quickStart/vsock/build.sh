...
if ! echo '#include <linux/vm_sockets.h>' | gcc -E -x c - >/dev/null 2>&1; then
    echo "linux/vm_sockets.h is required to build the vsock example rootfs." >&2
    echo "Install Linux kernel headers for your distribution, then rerun this script." >&2
    exit 1
fi
